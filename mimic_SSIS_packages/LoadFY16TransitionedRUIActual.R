...
# LoadFY16CustomerCountry - Run this package and make sure the dataframes dont have the same name
StgCustomerGeography <- stg

# Dataflow ETL - Customer Transition Date

	# Load Data - Customer Transition Date
	t1 <- sqlQuery(dcn,"
		Declare @TransitionDateStart Date
		Declare @TransitionDateEnd Date

		Select
			@TransitionDateStart  = Cast(Case When d.FiscalMonthName = 'July' Then s.TransitionDateStart Else d.FiscalMonthStartDate End As Date),
			@TransitionDateEnd  = Cast(s.TransitionDateEnd As Date)
		From
			(
				Select
					FiscalMonthName,
					Cast(FiscalMonthStartDate As Date) As FiscalMonthStartDate
				From
					StgDateMapping
				Where
					FiscalMonthPosition = -1
			) d
			Inner Join StgTransitionDateRangeYTD s On 
				d.FiscalMonthName = s.CompletedFiscalMonthName

		Delete
			StgCustomerTransitionDateYTD
		where
			TransitionDate Between @TransitionDateStart And @TransitionDateEnd 

		Select  @TransitionDateStart As TransitionDateStart, @TransitionDateEnd As TransitionDateEnd 
		");

# Dataflow ETL

	# Loop: Load data - Transitioned RUI - Transition Date

		# CustomerIDList

		# Loop: Load data - Transitioned RUI - Customer ID

			# Put query into variable

			# Load Data - Check if has data
